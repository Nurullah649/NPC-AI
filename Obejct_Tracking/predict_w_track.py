...
def filter_boxes(fused_boxes):
    filtered_boxes = []
    for i in range(len(fused_boxes)):
        keep = True
        for j in range(len(fused_boxes)):
            if i != j:
                iou = calculate_iou(fused_boxes[i], fused_boxes[j])
                if iou > 0.8:
                    box1_area = (fused_boxes[i][2] - fused_boxes[i][0]) * (fused_boxes[i][3] - fused_boxes[i][1])
                    box2_area = (fused_boxes[j][2] - fused_boxes[j][0]) * (fused_boxes[j][3] - fused_boxes[j][1])
                    if box1_area > box2_area:
                        keep = False
                        break
        if keep:
            filtered_boxes.append(fused_boxes[i])
...
def draw_boxes(frame, fused_boxes):
    ...
    num_objects = len(fused_boxes)
    cv2.putText(frame, f'Nesne Sayisi: {num_objects}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
def main():
    ...
    while cap.isOpened():
        ...
        fused_boxes = filter_boxes(fused_boxes)
