...
class RetrievalDBOW:
    ...
    def __call__(self, image, n):
        """Store the image into the frame buffer, ensuring it is a uint8 numpy array."""
        assert isinstance(image, np.ndarray), "Image must be a numpy array"
        if image.dtype != np.uint8:
            image = image.astype(np.uint8)
        if parse_shape(image, '_ _ RGB') != {'RGB': 3}:
            raise ValueError("Image shape must conform to '_ _ RGB'")
