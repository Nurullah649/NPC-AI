...
class DPVO:
    ...
    def __call__(self, tstamp, image, intrinsics):
        ...
        # Ensure intrinsics is of size [8] (add 4 zeros if it's of size [4])
        if intrinsics.shape[0] == 4:
            intrinsics = torch.cat(
                [intrinsics, torch.zeros(4, device=intrinsics.device)])  # 4 sıfır ekleyerek [8] boyutuna getiriyoruz

        # self.pg.intrinsics_'i 4 elemanla güncelleyin
        self.pg.intrinsics_[self.n] = intrinsics[:4] / self.RES
        # Ensure it is divided by RES as needed
        ...
        if self.n > 1:
            if self.cfg.MOTION_MODEL == 'DAMPED_LINEAR':
                # Son iki pozu SE3 objesine çevir
                P_nm1 = SE3(self.pg.poses_[self.n - 1])
                P_nm2 = SE3(self.pg.poses_[self.n - 2])

                # Zaman farklarını son üç timestamp ile hesapla
                # tlist[-3], tlist[-2], tlist[-1] sırasıyla önceki-önceki, önceki, güncel zamanlar
                a, b, c = self.tlist[-3], self.tlist[-2], self.tlist[-1]
                dt_prev = b - a
                dt_curr = c - b
                # Sıfıra bölünmeye karşı koruma ve aşırı büyük oranları sınırla
                if dt_prev <= 0:
                    fac = 1.0
                else:
                    fac = min(max(dt_curr / dt_prev, 0.1), 10.0)

                # Logaritmada delta pose
                delta_xi = (P_nm1 * P_nm2.inv()).log()
                # Damping katsayısını ve zaman faktörünü açılıma uygula
                xi = delta_xi * (self.cfg.MOTION_DAMPING * fac)
                # Yeni poz tahmini
                P_pred = SE3.exp(xi) * P_nm1
                self.pg.poses_[self.n] = P_pred.data

            elif self.cfg.MOTION_MODEL == 'CONSTANT_VELOCITY':
                # Sabit hız varsayımı: bir önceki delta’yı olduğu gibi uygula
                P_nm1 = SE3(self.pg.poses_[self.n - 1])
                P_nm2 = SE3(self.pg.poses_[self.n - 2])
                vel = P_nm1 * P_nm2.inv()
                P_pred = vel * P_nm1
