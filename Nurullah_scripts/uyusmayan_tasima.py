...
konum = "/home/npc-ai/Masaüstü/uyuşmayan_images"
konum3 = "/home/npc-ai/Masaüstü/uyuşmayan_labels"

# Hedef dizindeki .txt dosyalarını ve konum2'deki .jpg ve .png dosyalarını listele
dosyatxt_adlari = [dosya.rstrip('.txt') for dosya in os.listdir(hedef_dizin)]
dosyajpg_adlari = [os.path.splitext(dosya)[0] for dosya in os.listdir(konum2) if dosya.endswith('.jpg')]
dosyapng_adlari = [os.path.splitext(dosya)[0] for dosya in os.listdir(konum2) if dosya.endswith('.png')]
...
def jpg_tasi(dosyajpg_adlari,dosyatxt_adlari):
    ...
    for dosyajpg_adi in dosyajpg_adlari:
        if dosyajpg_adi not in dosyatxt_adlari:
            dosya_yolu = os.path.join(konum2, dosyajpg_adi + '.jpg')
            print('burda')
    ...
    for dosyapng_adi in dosyapng_adlari:
        if dosyapng_adi not in dosyatxt_adlari:
            dosya_yolu = os.path.join(konum2, dosyapng_adi + '.png')
            print('burda')
            shutil.move(dosya_yolu, konum)
            print(f"{dosya_yolu} dosyası {konum} konumuna taşındı.")
...
txt_tasi(dosyajpg_adlari,dosyatxt_adlari)