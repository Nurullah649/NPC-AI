#!/usr/bin/env python3
"""Open GT vs legacy-regression-fitted raw DPVO trajectory."""
...
def load(path: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    with path.open(newline="") as f:
        for row in csv.DictReader(f):
            ...
            pred.append([float(row["legacy_reg_x"]), float(row["legacy_reg_y"]), float(row["legacy_reg_z"])])
...
def main() -> None:
    ...
    ax.plot(
        ...
        color="orange",
        linewidth=1.8,
        label=f"DPVO + legacy regression | E={e:.1f}, RMSE={rmse:.1f}",
    )
    ...
    ax.set_title(f"GT vs DPVO legacy regression ({title_scope})")
    ax.legend()
    ax.grid(True)
    set_axes_equal(ax)
    plt.show()
