...
class VehicleTrackV2:
    ...
    def __init__(self, track_id: int, bbox: tuple, frame_idx: int,
        ...
        # Kamera hareketinden arindirilmis arac hizi. Kalman'in ham goruntu
        # koordinatindaki hizi kamera kaymasini da ogrenebildigi icin onu
        # tekrar homography ile tasimak hiz bileşenini iki kez sayiyordu.
        self.relative_velocity = np.zeros(2, dtype=np.float32)
        self.last_local_flow_velocity: Optional[np.ndarray] = None
        ...
        self.camera_relative_velocity_enabled = bool(
            motion_cfg.get("camera_relative_velocity_enabled", True)
        )
        self.relative_velocity_alpha = float(
            motion_cfg.get("relative_velocity_alpha", 0.65)
        )
        self.missed_velocity_decay = float(
            motion_cfg.get("missed_velocity_decay", 0.92)
        )
        self.local_flow_score_weight = float(
            motion_cfg.get("local_flow_score_weight", 0.50)
        )
    ...
    def predict(self, camera_transform: Optional[CameraTransform] = None) -> dict:
        ...
        if self.camera_relative_velocity_enabled:
            # Kamera transformu son gozlemi current frame'e getirir; yalniz
            # aracın zemine gore residual hizi association'a eklenir.
            association_predicted_center = camera_expected_center + self.relative_velocity
        elif camera_transform is not None and camera_transform.is_reliable:
    ...
    def compute_motion_score(
        ...
        """
        center_residual_vector = (det_center - pred_center).astype(np.float32)
        center_residual_px = float(np.linalg.norm(center_residual_vector))
        residual_vector = center_residual_vector
        residual_px = center_residual_px
        if self.last_local_flow_velocity is not None:
            flow_weight = min(1.0, max(0.0, self.local_flow_score_weight))
            residual_vector = (
                (1.0 - flow_weight) * center_residual_vector
                + flow_weight * self.last_local_flow_velocity
            ).astype(np.float32)
            # Skor yon vektorunun normu degil iki bagimsiz buyuklugun agirlikli
            # ortalamasidir; zit yonlu noise gercek hareketi sifirlamasin.
            residual_px = (
                (1.0 - flow_weight) * center_residual_px
                + flow_weight * float(np.linalg.norm(self.last_local_flow_velocity))
            )

        if self.camera_relative_velocity_enabled:
            instantaneous_velocity = residual_vector
            alpha = min(1.0, max(0.0, self.relative_velocity_alpha))
            if len(self.motion_scores) == 0:
                self.relative_velocity = instantaneous_velocity
            else:
                self.relative_velocity = (
                    alpha * instantaneous_velocity
                    + (1.0 - alpha) * self.relative_velocity
                ).astype(np.float32)
    ...
    def propagate_missed(self, camera_transform: Optional[CameraTransform]):
        ...
        if self.camera_relative_velocity_enabled:
            self.center = self.center + self.relative_velocity
            x1, y1, x2, y2 = self.bbox
            dx, dy = float(self.relative_velocity[0]), float(self.relative_velocity[1])
            self.bbox = (x1 + dx, y1 + dy, x2 + dx, y2 + dy)
            decay = min(1.0, max(0.0, self.missed_velocity_decay))
            self.relative_velocity *= decay
...
class VehicleTrackerV2:
    ...
    def __init__(self, config: dict):
        ...
        self.maha_gate_min_distance = float(
            motion_cfg.get("maha_gate_min_distance", 60.0)
        )
        self.maha_gate_diagonal_ratio = float(
            motion_cfg.get("maha_gate_diagonal_ratio", 0.25)
        )
        self.use_local_optical_flow = bool(
            motion_cfg.get("use_local_optical_flow", False)
        )
        self.local_flow_weight = float(motion_cfg.get("local_flow_weight", 0.50))
        self.local_flow_min_points = int(motion_cfg.get("local_flow_min_points", 4))
        self.local_flow_max_error = float(motion_cfg.get("local_flow_max_error", 20.0))
        self.local_flow_max_mad = float(motion_cfg.get("local_flow_max_mad", 20.0))
        self.local_flow_max_residual = float(
            motion_cfg.get("local_flow_max_residual", 250.0)
        )
        self.local_flow_max_track_age = int(
            motion_cfg.get("local_flow_max_track_age", 2)
        )
        self.deduplicate_vehicle_detections = bool(
            motion_cfg.get("deduplicate_vehicle_detections", True)
        )
        self.duplicate_iou_threshold = float(
            motion_cfg.get("duplicate_iou_threshold", 0.85)
        )
    ...
    def _deduplicate_vehicles(self, detections: list) -> list:
        """Ayni araca ait neredeyse ayni YOLO kutularini tekilleştir."""
        if not self.deduplicate_vehicle_detections:
            return detections

        vehicle_indices = [
            index for index, det in enumerate(detections) if det.get("cls") == 0
        ]
        ordered = sorted(
            vehicle_indices,
            key=lambda index: float(detections[index].get("conf", 0.0)),
            reverse=True,
        )
        kept = []
        suppressed = set()
        for index in ordered:
            ...
            if bbox is None:
                continue
            if any(
                _box_iou(bbox, detections[kept_index]["bbox"])
        ...
        if not suppressed:
            return detections
        return [det for index, det in enumerate(detections) if index not in suppressed]

    def _estimate_local_relative_velocity(
        self,
        track: VehicleTrackV2,
        prev_gray: np.ndarray,
        gray: np.ndarray,
        camera_transform: Optional[CameraTransform],
    ) -> Optional[np.ndarray]:
        """Track ROI optical flow'undan kamera-harici hizi tahmin et."""
        if (
            not self.use_local_optical_flow
            or prev_gray is None
            or gray is None
            or camera_transform is None
            or not camera_transform.is_reliable
            or track.time_since_update > self.local_flow_max_track_age
        ):
            return None

        height, width = prev_gray.shape[:2]
        x1, y1, x2, y2 = track.bbox
        pad_x = max(3.0, (x2 - x1) * 0.08)
        pad_y = max(3.0, (y2 - y1) * 0.08)
        ix1 = max(0, int(np.floor(x1 - pad_x)))
        iy1 = max(0, int(np.floor(y1 - pad_y)))
        ix2 = min(width, int(np.ceil(x2 + pad_x)))
        iy2 = min(height, int(np.ceil(y2 + pad_y)))
        ...
        if points_prev is None or len(points_prev) < self.local_flow_min_points:
            return None

        points_curr, status_forward, errors = cv2.calcOpticalFlowPyrLK(
            prev_gray,
            gray,
            points_prev,
            None,
        ...
        if points_curr is None or status_forward is None:
            return None
        ...
        p0 = points_prev.reshape(-1, 2)
        p1 = points_curr.reshape(-1, 2)
        pb = points_back.reshape(-1, 2)
        valid = status_forward.reshape(-1).astype(bool) & status_back.reshape(-1).astype(bool)
        valid &= np.linalg.norm(pb - p0, axis=1) <= 2.0
        if errors is not None:
            valid &= errors.reshape(-1) <= self.local_flow_max_error
        p0 = p0[valid]
        p1 = p1[valid]
        if len(p0) < self.local_flow_min_points:
            return None

        camera_points = np.array(
            [camera_transform.apply_to_point(point) for point in p0],
            dtype=np.float32,
        )
        residuals = p1 - camera_points
        median = np.median(residuals, axis=0).astype(np.float32)
        deviations = np.linalg.norm(residuals - median, axis=1)
        mad = float(np.median(deviations))
        if mad > self.local_flow_max_mad:
            return None
        if float(np.linalg.norm(median)) > self.local_flow_max_residual:
            return None
        return median
    ...
    def _build_cost(
        ...
        for di in range(n_dets):
            ...
            for ti in range(n_tracks):
                ...
                maha_min_distance = max(
                    self.maha_gate_min_distance,
                    _bbox_diagonal(pred_bbox) * self.maha_gate_diagonal_ratio,
                )
                if maha > self.maha_gate_threshold and dist > maha_min_distance:
                    c = 1e6
    ...
    def update(
        ...
        for tr in active_tracks:
            local_velocity = self._estimate_local_relative_velocity(
                tr, previous_gray, gray, camera_transform,
            )
            tr.last_local_flow_velocity = local_velocity
            if local_velocity is not None:
                weight = min(1.0, max(0.0, self.local_flow_weight))
                tr.relative_velocity = (
                    weight * local_velocity
                    + (1.0 - weight) * tr.relative_velocity
                ).astype(np.float32)
